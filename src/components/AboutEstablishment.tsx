...
export function AboutEstablishment() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (statsRefs.current.length > 0)
        gsap.set(statsRefs.current.filter(Boolean), {
          opacity: 1,
          y: 0,
          scale: 1,
        });
      if (imageRef.current)
        gsap.set(imageRef.current, { opacity: 1, rotateY: 0, x: 0 });
  ...
  const stats = [
    {
      icon: Calendar,
      labelKey: "establishmentStat1",
      gradient: "from-[#035938] to-[#52BC88]",
    },
    {
      icon: MapPin,
      labelKey: "establishmentStat2",
      gradient: "from-[#52BC88] to-[#F1BC28]",
    },
    {
      icon: Users,
      labelKey: "establishmentStat3",
      gradient: "from-[#F1BC28] to-[#035938]",
    },
    {
      icon: Award,
      labelKey: "establishmentStat4",
      gradient: "from-[#035938] to-[#52BC88]",
    },
    {
      icon: Zap,
      labelKey: "establishmentStat5",
      gradient: "from-[#52BC88] to-[#F1BC28]",
    },
    {
      icon: Globe,
      labelKey: "establishmentStat6",
      gradient: "from-[#F1BC28] to-[#035938]",
    },
  ];
          ...
          {stats.map((stat, index) => (
            ...
            >
              ...
              <div className="relative z-10">
                <div
                  className={`w-14 h-14 mx-auto rounded-xl bg-gradient-to-br ${stat.gradient} flex items-center justify-center mb-3 transform group-hover:rotate-12 group-hover:scale-110 transition-all duration-500 shadow-lg`}
                >
              ...
              <div
                className={`absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r ${stat.gradient} transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500`}
              />
            </div>
          ))}
        </div>
